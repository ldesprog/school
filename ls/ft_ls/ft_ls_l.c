...
int		ft_ls_l_2(int i, char ***out, struct stat stat_file, char *name)
{
	...
	out[i] = (char **)ft_malloc(sizeof(char *), 12);
	...
	out[i][4] = ft_itoa((int)stat_file.st_size);
	file2 = ft_strsplit(ctime(&stat_file.st_mtime), ' ');
	out[i][5] = file2[2];
	out[i][6] = file2[1];
	out[i][7] = ft_verif_time(stat_file);
	out[i][8] = name;
	out[i][9] = NULL;
	...
	while ((file = readdir(dir)))
	{
		if ((opt.a == 0 && file->d_name[0] != '.') || opt.a == 1)
		{
			...
			if (S_ISLNK(stat_file.st_mode))
			{
				...
				out[i][9] = (char *)malloc(sizeof(char) * (strlen(name_link) + 4));
				strcat(strcat(out[i][9], "-> "), (name_link));
				out[i][10] = NULL;
			}
		}
	}
